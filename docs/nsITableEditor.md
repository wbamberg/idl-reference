...
<pre> Delete just the cell contents  
...
<pre> Delete cell elements as well as contents  
...
<pre> Create a new TD or TH element, the opposite type of the supplied aSourceCell  
...
<pre> Merges contents of all selected cells  
...
### splitTableCell() ###
<pre> Split a cell that has rowspan and/or colspan > 0  
...
<pre> Scan through all rows and add cells as needed so   
...
<pre> Get the row an column index from the layout's cellmap  
If aCell is null, it will try to find enclosing table of selection anchor  
  
  
</pre>
### getTableSize(aTable, aRowCount, aColCount) ###
<pre> Get the number of rows and columns in a table from the layout's cellmap  
...
<pre> Get a cell element at cellmap grid coordinates  
...
<pre> Get a cell at cellmap grid coordinates and associated data  
A cell that spans across multiple cellmap locations will  
  be returned multiple times, once for each location it occupies  
...
<pre> Get the first row element in a table  
...
<pre> Get the next row element starting the search from aTableElement  
...
<pre> Preferred direction to search for neighboring cell  
...
</pre><pre> Reset a selected cell or collapsed selection (the caret) after table editing  
...
<pre> Examine the current selection and find  
...
<pre> Generally used after GetSelectedOrParentTableElement  
...
<pre> Get first selected element from first selection range.  
...
<pre> Get first selected element in the table  
...
<pre> Get next selected cell element from first selection range.  
Assumes cell-selection model where each cell  
is in a separate range (selection parent node is table row)  
